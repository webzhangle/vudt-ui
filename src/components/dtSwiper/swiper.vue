<template>
  <div class="dt-slider">
    <div class="dt-swiper" :style="{height: xheight}">
      <slot></slot>
      ...
      :data-index="index">
        <a href="javascript:">
          <div class="dt-img" :style="{backgroundImage: buildBackgroundUrl(item)}"></div>
          <p class="dt-swiper-desc" v-if="showDescMask">{{ item.title }}</p>
        </a>
      </div>

      <div
      v-if="listTwoLoopItem.length > 0"
      class="dt-swiper-item dt-swiper-item-clone"
    ...
    <div :class="[dotsClass, 'dt-indicator', `dt-indicator-${dotsPosition}`]" v-show="showDots">
      <a href="javascript:" v-for="key in length">
        <i class="dt-icon-dot" :class="{'active': key - 1 === current}"></i>
      </a>
    </div>
  </div>
</template>
